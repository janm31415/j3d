...
namespace
  ...
  template <class TCHAR>
  const ogt_vox_scene* load_vox_scene(const TCHAR* filename, uint32_t scene_read_flags = 0)
    {
    file_opener<TCHAR> fo;
    // open the file
    FILE* fp = fo(filename, "rb");
    if (!fp)
      return NULL;
  ...
  template <class TCHAR>
  void save_vox_scene(const TCHAR* pcFilename, const ogt_vox_scene* scene)
    {
  ...
  template <class TCHAR>
  bool _read_vox(const TCHAR* filename, std::vector<jtk::vec3<float>>& vertices, std::vector<uint32_t>& clrs, std::vector<jtk::vec3<uint32_t>>& triangles)
    {
    vertices.clear();
    clrs.clear();
    triangles.clear();
  ...
  template <class TCHAR>
  bool _write_vox(const TCHAR* filename,
...
bool read_vox(const char* filename, std::vector<jtk::vec3<float>>& vertices, std::vector<uint32_t>& clrs, std::vector<jtk::vec3<uint32_t>>& triangles)
  {
  return _read_vox<char>(filename, vertices, clrs, triangles);
  }

bool write_vox(const char* filename, const std::vector<jtk::vec3<float>>& vertices, const std::vector<jtk::vec3<float>>& clrs, const std::vector<jtk::vec3<uint32_t>>& triangles, const std::vector<jtk::vec3<jtk::vec2<float>>>& uv, const jtk::image<uint32_t>& texture, uint32_t max_dim)
  {
  return _write_vox<char>(filename, vertices, clrs, triangles, uv, texture, max_dim);
  }
